...
async function getMonthlyChatCount(db: ReturnType<typeof getDb>, userId: number) {
  ...
  const result = await db
    ...
    .from(chats)
    .where(and(eq(chats.userId, userId), gte(chats.createdAt, startOfMonth)));
...
export async function handleKirimiWebhook(payload: any) {
  const db = getDb();
  ...
  if (event === "message" || event === "msg") {
    ...
    if (quotaStatus.status === "exhausted") {
      if (user.extraCredits > 0) {
        ...
        await db
          .update(chats)
          .set({ lastMessage: PAYWALL_MESSAGE, status: "ai_handled" })
          .where(eq(chats.id, chat.id));
...
Akhiri dengan semangat dan emoji!`;
  ...
  if (event === "connection.connected" || event === "connection.disconnected") {
    const { deviceId: kirimiDeviceId } = payload;
